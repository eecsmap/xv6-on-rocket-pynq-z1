...
int main(void)
{
	...
	/*
	 * Explicitly force UART0 TX/RX enable. ps7_init() should already do
	 * this (CR=0x17), but on this board it's observed left disabled
	 * (CR reads back 0x128, TXDIS/RXDIS set) by the time we get here,
	 * silently dropping all fsbl_printf output into an overflowing TX
	 * FIFO. Force it directly rather than chasing why ps7_init's write
	 * isn't sticking.
	 */
	Xil_Out32(0xE0000000, 0x00000017);
